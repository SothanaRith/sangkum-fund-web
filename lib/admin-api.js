...
export const adminAnnouncementsAPI = {
  getAll: async (page = 0, size = 10) => {
    const { data } = await apiClient.get(`/api/admin/announcements?page=${page}&size=${size}`);
    return data;
  },

  getById: async (id) => {
    const { data } = await apiClient.get(`/api/admin/announcements/${id}`);
    return data;
  },

  create: async (announcementData) => {
    const { data } = await apiClient.post('/api/admin/announcements', announcementData);
    return data;
  },

  update: async (id, announcementData) => {
    const { data } = await apiClient.put(`/api/admin/announcements/${id}`, announcementData);
    return data;
  },
  ...
  getByEvent: async (eventId) => {
    const { data } = await apiClient.get(`/api/admin/announcements/event/${eventId}`);
    return data;
  },

  getByCharity: async (charityId) => {
    const { data } = await apiClient.get(`/api/admin/announcements/charity/${charityId}`);
    return data;
  },
};
