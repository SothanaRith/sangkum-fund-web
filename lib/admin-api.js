...
export const adminAnnouncementsAPI = {
  getAll: async (page = 0, size = 10) => {
    const { data } = await apiClient.get(`/api/announcements?page=${page}&size=${size}`);
    return data;
  },
  ...
  getByEvent: async (eventId) => {
    const { data } = await apiClient.get(`/api/announcements/event/${eventId}`);
    return data;
  },

  getByCharity: async (charityId) => {
    const { data } = await apiClient.get(`/api/announcements/charity/${charityId}`);
    return data;
  },
};
