      ...
      if (typeof window !== 'undefined') {
        ...
        if (refreshToken) {
          try {
            ...
            // Save new tokens in localStorage and sync cookies for middleware
...
export const donationsAPI = {
  ...
  verifyBakong: async (verifyData) => {
    const { data } = await apiClient.post('/api/donations/verify-bakong', verifyData);
    return data;
  },
...
export const announcementsAPI = {
  ...
  addComment: async (announcementId, commentData) => {
    const { data } = await apiClient.post(`/api/announcements/${announcementId}/comments`, commentData);
    return data;
  },
  ...
  // Comment Reactions
  addCommentReaction: async (announcementId, commentId, reactionType) => {
    const { data } = await apiClient.post(`/api/announcements/${announcementId}/comments/${commentId}/reactions`, { reactionType });
    return data;
  },

  removeCommentReaction: async (announcementId, commentId) => {
    const { data } = await apiClient.delete(`/api/announcements/${announcementId}/comments/${commentId}/reactions`);
    return data;
  },
...
export const dashboardAPI = {
  getAnalytics: async (dateRange = 'all') => {
    const params = dateRange !== 'all' ? { dateRange } : {};
    const { data } = await apiClient.get('/api/dashboard/analytics', { params });
    return data;
  },
};
